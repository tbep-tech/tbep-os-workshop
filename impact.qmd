...
![Table 4b](img/tidyex4b.png){fig-lo}
:::

Only the first table is tidy - each variable has its own column, each observation has its own row, and each value has its own cell. Table 2 violates the second rule, Table 3 violates the third rule, and tables 4a and 4b violate the first rule.
...
1. Click the submit button at the top to start the process of entering your data
1. Add the relevant metadata information in the forms (because this is a test archive, not all will be entered)
1. Save the dataset when you're done and marvel at your work