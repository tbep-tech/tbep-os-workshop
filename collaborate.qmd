...
Workflow management in the real world - using GitHub to collaborate.  Here we present some examples from the Tampa Bay Estuary Program [State of the Bay](https://tbep.org/estuary/state-of-the-bay/) report and [water quality report card](https://tbep.org/water-quality-report-card/). 
...
1. Open [GitHub](https://www.github.com) in a web browser and have one person create a new repository (the big, green "New" button in Repositories). Add each member to the repository after it's created (hint: Settings -> Collaborators)
