...
<h3>`r fa('chalkboard-user')` Watch and learn</h3> {#sec-realworld}

Workflow management in the real world - using GitHub to collaborate.  Here we present some examples from the Tampa Bay [State of the Bay](https://tbep.org/estuary/state-of-the-bay/) report and [water quality report card](https://tbep.org/water-quality-report-card/). 
...
1. Open [GitHub](https://www.github.com) in a web browser and have one person create a new repository (the big, green "New" button in Repositories) and add each member to the repository after it's created (hint: Settings -> Collaborators)
...
1. Assign issues to different members of the repository to do something to the new files (Hint: on the right menu, select "Assignees").  Add the issue to the project board before (Hint: on the right menu, select "Projects" and click the new project).
